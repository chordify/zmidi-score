...
toCSV :: QBinSelection -> NSWPStore -> Either String ByteString
toCSV s n = 
  let -- ignore rotation in this export (rotation is not annotated in MIDI data)
      ...
      f (GTMR ts _, ps) = case M.lookup ts ps of
           Just p  -> Right $ toRNSWProfWithTS 0 ts s p -- no rotation
...
toRNSWProfWithTS :: Rot -> TimeSig -> QBinSelection -> NSWProf -> RNSWProf
toRNSWProfWithTS r ts s = RNSWProf ts . filterToList r s ts
            
-- writes a CSV Header to a file 
genHeader :: QBinSelection -> String
genHeader m = (++ "\n") . intercalate "," $ "meter" : 
              (map printKey . fromJust . M.lookup (TimeSig 4 4 0 0) $ m)
            
printKey :: BarRat -> String
printKey (BarRat br) = show (numerator br) ++ "." ++ show (denominator br)            